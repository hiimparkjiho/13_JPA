...
*
* - name: 참조하는 테이블의 컬럼명
* - referenceColumnName: 참조되는 컬럼명을 지정한다.
* - nullable : 참조하는 테이블의 컬럼에 null 값을 허용할지 여부를 지정한다.
* - unique : 참조하는 테이블의 컬럼에 유일성 제약 조건을 추가할지 여부를 지정한다.
* - insertable : 새로운 엔티티가 저장될 때, 이 참조 컬럼이 sql insert문에 포함될지 여부를 지정한다.
* - updatable : 엔티티가 업데이트 될 때, 이 참조 컬럼이 sql update문에 포함될지 여부를 지정한다.
* - table : 참조하는 테이블의 이름을 지정한다.
* - foreignkey : 참조하는 테이블에 생성될 외래 키에 대한 추가 정보를 지정한다.
*
* @ManyToOne은 다대일 지정 관계에서 사용되는 어노테이션이다.
* @ManToOne에서 사용할 수 있는 속성들은 다음과 같다.
*
