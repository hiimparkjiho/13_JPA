...
public class ManyToOneAssociationTests {
    ...
    * - 1:N(OneToMany) 연관관계
    * - 1:1(OneToOne) 연관관계
    * - N:N(ManyToMany) 연관관계    // 서로가 서로를 참조하는 관계이다 보니 고려를 하지 않는 것이 좋다
    *
    * 방향에 따른 분류
    ...
    * */
    ...
    // 연관관계를 가지는 엔티티를 조회하는 방법은 객체 그래프 탐색(객체 연관 관계를 사용한 조회), 객체 지향 쿼리(jpql) 사용이 있다.

    @Test
    void 디테일_연관관계_객체_그래프_탐색을_이용한_조회_테스트(){
        // given
        int menuCode = 15;
    ...
    /*
    * JPQL은 java Persistence Query Langueage의 약자로, 객체 지향 쿼리 언어 중 하나이다.
    ...
    * */
    ...
    void 다대일_연관관계_객체지향쿼리_사용한_카테고리_이름_조회_테스트(){
        // given
        String jpql = "SELECT c.categoryName FROM menu_and_category m JOIN m.category c WHERE m.menuCode = 15"; // MenuAndCategory 클래스에 조인해줬기 때문에 on을 쓰지 않는다.
    ...
    * 영속성 전이랑 특정 엔티티를 영속화 할 때 연관된 엔티티도 함께 영속화한다는 의미이다.
    * cascade=Cascade.PERSIST를 설정하면 MenuAndCategory 엔티티를 영속화 할 때 Cateogry 엔티티도 함께 영속화하게 된다.
    * */

    @Test
    void 다대일_연관관계_객체_삽입_테스트(){
        MenuAndCategory menuAndCategory = new MenuAndCategory();
        menuAndCategory.setMenuCode(9997656);
        menuAndCategory.setMenuName("멸치빙수");
        menuAndCategory.setMenuPrice(30000);

        Category category = new Category();
        category.setCategoryCode(3339);
        ...
        //then
        MenuAndCategory foundMenuAndCategory = entityManager.find(MenuAndCategory.class, 9997656);
        assertEquals(9997656, foundMenuAndCategory.getMenuCode());
        assertEquals(3339, foundMenuAndCategory.getCategory().getCategoryCode());
    }
    ...
    void 영속성_삭제_테스트(){
        ...
        MenuAndCategory menuAndCategory = entityManager.find(MenuAndCategory.class,999765769);
        ...
        transaction.commit();
        MenuAndCategory deletMenu = entityManager.find(MenuAndCategory.class,999765769);
        // cascade remove는 부모가 삭제된다면 자식까지 모두 삭제된다 DB규칙에 위배되기 때문이다.
    ...
    void Refresh_테스트(){
         ...
         * */
        MenuAndCategory menuAndCategory = entityManager.find(MenuAndCategory.class,999765768);
    ...
    void Merge_update_테스트(){
        ...
        Category category = entityManager.find(Category.class,33341);
    ...
    void detach_테스트(){
        ...
        MenuAndCategory menuAndCategory = entityManager.find(MenuAndCategory.class, 999765771);
