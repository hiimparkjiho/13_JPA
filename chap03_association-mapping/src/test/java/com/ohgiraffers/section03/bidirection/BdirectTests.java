...
public class BdirectTests {
    ...
    /*
    * 데이터베이스의 테이블은 외래키 하나로 양방향 조회가 가능하지만 객체는 서로 다른 두 단방향 참조를 합쳐서 양방향이라고 하낟
    * 따라서 두개의 연관 관계 중 연관 관계의 주인을 정하고, 주인이 아닌 연관 관계를 하나 더 추가하는 방식으로 작성하게 된다.
    ...
    * 비즈니스 중요도를 배제하고 단순히 외래키 관리자의 이미를 부여해야 한다.
    ...
    * 양방향 연관관계를 설정하고 흔히 하는 실수이므로 반드시 menu 엔티티에 카테고리 정보를 추가한다
    * */
    ...
    public void 양방향_연관관계_주인이_아닌_객체를_이용한_삽입_테스트() {
        ...
        insertMenu.setCategory(category);
