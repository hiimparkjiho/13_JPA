...
public class SimpleJPQLTests {
    ...
    /*
    * jpql(java persistence language)
    ...
    * */
    ...
    public void Query를_이용한_단일_메뉴_조회_테스트(){
        ...
        //when
        String jqpl = "SELECT m.menuName from menu_section01 as m where m.menuCode = 7";
    ...
    void TypedQuery를_이용한_단일행_조회_테스트(){
        ...
        Menu findMenu = query.getSingleResult();

        Assertions.assertEquals(7, findMenu.getMenuCode());
    ...
    void TypedQuery를_이용한_다중행_조회_테스트(){
        ...
        List<Menu> findMenu = query.getResultList();

        Assertions.assertNotNull(findMenu);
        findMenu.forEach(System.out::println);
    }

    @Test
    public void Query를_이용한_다중행_메뉴_조회_테스트(){

        //when
        String jqpl = "SELECT m.menuName from menu_section01 as m";
        Query query = entityManager.createQuery(jqpl);

        List<Object> findMenuList = query.getResultList();

        Assertions.assertNotNull(findMenuList);
        findMenuList.forEach(System.out::println);
    }
