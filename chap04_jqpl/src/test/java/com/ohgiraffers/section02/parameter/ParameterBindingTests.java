...
public class ParameterBindingTests {
    ...
    * */
    ...
    void 이름_기준_파라미터_바인딩_메뉴_목록_조회_테스트(){
        //given
        String menuNameParamter = "한우딸기국밥";

        //when
        String jpql = "SELECT m FROM menu_section02 m WHERE m.menuName = :menuName";

        List<Menu> menuList = entityManager.createQuery(jpql, Menu.class).setParameter("menuName", menuNameParamter).getResultList();
    ...
    void 위치_기준_파라미터_바인딩_메뉴_목록_조회_테스트(){
        ...
        List<Menu> menuList = entityManager.createQuery(jpql, Menu.class)
                .setParameter(1, menuNameParamter).getResultList();
