...
public class ProjectionTests {
    ...
    * */
    ...
    void 단일_엔티티_프로젝션_테스트(){
        ...
        // mybatis에서 조회해온 결과는 영속성 컨텍스트에 들어가지 않기 때문에 update문도 돌아가지 않는다.
        // mybatis에서 조회해온 값을 update하기 위해선 persist를 사용해야 한다.
        transaction.begin();
    ...
    void 임베디드_타입_프로젝션_테스트(){
        ...
        // embedded 는 복합키 설정을 위해 자주 사용.(매핑 테이블)
    }
     ...
     * 그때 TypeQuery대신 Query를 사용하여 Object[]로 행의 정보를 반환받아 사용한다.
     * */
    @Test
    void Query를_이용한_스칼라_타입_프로젝션_테스트(){

        //when
        String jpql = "SELECT c.categoryCode, c.categoryName FROM category_section03 AS c";
        // 반환타입을 모르고 컬럼이 여러개일 때 Query를 사용하곤 한다
    ...
    void new_명령어를_활용한_프로젝션_테스트(){
        ...
        //when
        String jpql = "SELECT new com.ohgiraffers.section03.projection.CategoryInfo(c.categoryCode, c.categoryName) FROM category_section03 AS c";
