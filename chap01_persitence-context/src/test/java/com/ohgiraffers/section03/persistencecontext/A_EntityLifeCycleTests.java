...
public class A_EntityLifeCycleTests {
    ...
    * */
    ...
    public void 비영속_테스트(){
        // given
        Menu foundMenu = entityManager.find(Menu.class, 11);    // PK가 11번인 메뉴 호출

        // 객체만 생성하면 영속성 컨택스트나 데이터베이스와 관련 없는 비영속 상태이다.
        ...
        //when
        boolean isTrue = (foundMenu == newMenu);    // 주소값이 다르기 때문에 false가 나옴
        //then
    ...
    void 영속성_연속_조회_테스트(){
        /*
        * 엔티티 매니저가 영속성 컨택스트에 엔티티 객체를 저장(persist)하면 영속성 컨택스트가 엔티티 객체를 관리하게 되고
        * 이를 영속 상태라고 한다. Find(), jpql을 사용한 조회도 영속 상태가 된다.
        * */

        // jpa의 단점을 보완하기 위해 사용하는 jpql ?
    ...
    void 준영속성_detach_테스트(){
        ...
        /*
        * 영속성 컨택스트가 관리하던 엔티티 객체를 관리하지 않는 상태가 된다면 준영속 상태가 된다.
    ...
    @Test
    void 준영속성_clear_테스트(){  //영속성 컨텍스트를 초기화.
    ...
    void close_테스트(){
        ...
        //when
        entityManager.close();  // 엔티티 매니저를 꺼버리는 것 다시 쓰려면 다시 만들어줘야 함
    ...
    * 만약 1차 캐시에 엔티티가 없으면 데이터베이스에서 엔티티를 조회하고 1차 캐사에 저장한다.
