...
public class A_EntityManagerLifeCycleTests {
    /*
    * 엔티티 매니저 팩토리(EntityManagerFactory)란? DB랑 커넥션 맺은 형태
    ...
    * 따라서 데이터베이스를 사용하는 에플리케이션 당 한 개의 EntityManagerFactory
    * 라이프 사이클이
    * */

    /*
    * 엔티티 매니저(Entity Manager)란? 메모리 상에서 관리되고 있는 데이터베이스, DB조작을 위해 필요한 인터페이스?
    ...
    *
    * 영속성 컨텍스트(Persitance context)란?
    * 엔티티 매니저를 통해 엔티티를 저장하거나 조회하면 엔티티 매니저는 영속성 컨택스트에 엔티티를 보관하고 관리한다.
    * 영속성 컨텍스트는 엔티티를 key-value 방식으로 저장하는 저장소이다.
    * 영속성 컨택스트는 엔티티 매니저를 생성할 때 하나 만들어진다.
    * 그리고 엔티티 매니저를 통해서 영속성 컨택스트에 접근할 수 있고, 또 관리할 수 있다.
    * */
    ...
    public static void initFactory(){
        entityManagerFactory = Persistence.createEntityManagerFactory(("japtest"));
    }
    ...
    @AfterAll
    public static void colseFactory(){
