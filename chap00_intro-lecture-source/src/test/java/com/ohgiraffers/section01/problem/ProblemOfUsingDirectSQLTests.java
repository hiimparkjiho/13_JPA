...
class ProblemOfUsingDirectSQLTests {
    ...
    * 1. 데이터 반환, SQL 작성, JDBC API 코드 등의 중복 작성(개발 시간 증가, 유지보수성 저하)
    * 2. SQL에 의존하여 개발
    ...
    * */
    ...
    @DisplayName("직접 SQL을 작성하여 신규 메뉴를 작성할 때 발생하는 문제 확인")
    @Test
    void testDirectInsertSql() throws SQLException {
        Menu menu = new Menu();
        menu.setMenuName("민트초코 짜장면");
    ...
    /*
    * 2. SQL에 의존하여 개발
    * 요구사항의 변경에 따라 에플리케이션의 수정이 SQL의 수정으로도 이어진다.
    ...
    * */
    ...
    void testAssociatedObject() throws SQLException {
        ...
        while(rest.next()){
            ...
            menuCategory.setMenuCode(rest.getInt("MENU_CODE"));
            menuCategory.setMenuName(rest.getString("MENU_NAME"));
            menuCategory.setMenuPrice(rest.getInt("MENU_PRICE"));
            menuCategory.setCategory(new Category(rest.getInt("CATEGORY_CODE"), rest.getString("CATEGORY_NAME")));
            menuCategory.setOrderableStatus(rest.getString("ORDERABLE_STATUS"));

            menuCategoryList.add(menuCategory);
        }
        Assertions.assertNotNull(menuCategoryList);
    ...
    * 3-1 상속 문제
    * 객체 지향 언어의 상속 개념과 유사한 것이 데이터베이스의 서브타입 엔티티이다.
    * 유사한 것 같지만 다른 부분은 데이터베이스의 상속은 상속 개념을 데이터로 추상화해서 슈퍼타임과 서브타임으로 구분하고,
    * 슈퍼타입의 모든 속성을 서브타입이 공윻지 못하여 물리적으로 다른 테이블의 분리가 된 형태이다.
    * (설계에 따라서는 속성으로 추가되기도 한다.)
    * 하지만 객체지향의 슈퍼타입의 속성을 공유해서 사용하므로 여기서 패러다임 불일치 현상이 발생한다.
    *
    * 법인 회원과 일반 회원이라는 두가지 회원을 추상화하여 회원이라는 슈퍼타입 엔티티를 도출하고
    * 서브타입 엔티티로 법인 회원과 일반 회원이 존재하는 상황인 경우
    * 물리적으로 회원과 법인 회원 테이블로 분리된 상황에서는 각각 insert 구문을 수행해야 한다.
    * insert into 회원..
    * insert into 법인회원
    * 하지만 jap를 이용하여 상속을 구현하는 경우에는
    * entityManager.parseInt(법인회원)
    * 이렇게 한 번에 해결하는 것이 가능하다.
    * */

    /*
    * 3-2. 연관관계 문제, 객체 그래프 탐색 문제
    * 객체지향에서 말하는 가지고 있는 (associatation 연관관계 혹은 Collection 연관관계) 경우 데이터 베이스 저장 구조오 다른 형태이다.
    ...
    *    public class Menu {
    ...
    void testEquals() throws SQLException {
        ...
        // 저장된 주소값이 다르기 때문에 false
        // JPA에서는 true 영속성 컨텍스트 때문에?
